...
import { getFunnelReport } from "@/lib/ga4-lms";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, fetchedAt } = await getFunnelReport(propertyId, property, { start, end });
    return jsonWithTimestamp(data, fetchedAt);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    console.error("GA4 funnel report fetch failed:", message);
